...
namespace Tests\Unit\AleBatistella\BlingErpApi\Entities\FormasDePagamentos;
...
/**
 * Teste de `FormasDePagamentos`.
 */
class FormasDePagamentosTest extends TestCase
{
     ...
     */
    private function getInstance(IBlingRepository $repository): FormasDePagamentos
    {
        return new FormasDePagamentos($repository);
    }
        ...
        $repository->expects($this->once())
            ...
            ->with(
                $this->callback(
                    ...
                    $requestOptions->endpoint === "formas-pagamentos/$idContrato"
                )
            )
    ...
    public function testShouldGetSuccessfully(): void
    {
        ...
        $repository->expects($this->once())
            ...
            ->with(
                $this->callback(
                    fn(RequestOptions $requestOptions) =>
                    $requestOptions->endpoint === "formas-pagamentos"
